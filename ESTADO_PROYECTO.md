...
14. **Detección de posibles duplicados al registrar por WhatsApp** — antes de guardar un gasto simple (no en cuotas), se compara contra los gastos del mes (misma descripción normalizada + mismo monto, sin importar quién lo cargó). Si hay coincidencia, no se guarda: se responde con una advertencia ("ya tenés registrado $X en 'Y' este mes") y se guarda una confirmación pendiente por número de WhatsApp (15 min de validez). Si la persona responde "sí"/"dale"/"confirmo", recién ahí se registra.
...
- **Las confirmaciones pendientes de posibles duplicados también viven en memoria** (`server/services/confirmacionesPendientesService.js`, 15 min de TTL) — si el servidor se reinicia justo entre la advertencia y la confirmación, el "sí" se procesa como un mensaje nuevo en vez de confirmar el gasto.
